...
void AudioMixer_queueSound(wavedata_t *pSound)
{
    ...
    for(int i = 0; i < MAX_SOUND_BITES; i++) {
        ...
        if(i == MAX_SOUND_BITES - 1) {
            printf("No free space in soundBites[]\n");
        }
    }
...
static void fillPlaybackBuffer(short *buff, unsigned long size)
{
	/*
	 * REVISIT: Implement this
    ...
    // 1. clear playbackBuffer
    memset(buff, 0, sizeof(short) * size);

    // 2. make accesses to soundBites[] threadsafe (use mutex)
    pthread_mutex_lock(&audioMutex);

    // 3. loop thru soundBites[] and add all waiting sounds to the playbackBuffer
    for(int i = 0; i < MAX_SOUND_BITES; i++) {
        unsigned long playbackBufferLocation = 0;

        if(soundBites[i].pSound == NULL) {
            continue;
        } else {
            // we found a sound waiting to be played
            int j = 0;
